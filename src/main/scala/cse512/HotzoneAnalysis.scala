...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val resultSet = spark.sql("select rectangle, COUNT(point) from joinResult GROUP BY rectangle ORDER BY rectangle")
    val resultSetCompressed = resultSet.coalesce(1)
    //resultSet.collect.foreach(println)

    return resultSetCompressed
  }
